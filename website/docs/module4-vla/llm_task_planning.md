...
The emergence of Large Language Models (LLMs) has revolutionized how AI systems understand and respond to human language. In robotics, LLMs are transforming task planning, enabling robots to interpret high-level, abstract human commands and translate them into a sequence of executable robot actions. This chapter explores how LLMs are being leveraged for robotic task planning within the Vision-Language-Action (VLA) paradigm.

## Traditional Robotic Task Planning

Historically, robotic task planning has often relied on:
...
-   **Example Prompt**:
    ...
    Given the following actions: [open_gripper(), close_gripper(), move_to(location), pick_up(object), place(object, location)]
    Task: "Pick up the red block and place it in the blue bin."
    Actions:
    1. move_to(red_block)
    2. pick_up(red_block)
    3. move_to(blue_bin)
    ...
    Actions:
    1. // LLM generates steps for making coffee
    ```
-   **Benefit**: Highly flexible and adapts to new tasks without retraining.
...
-   **Benefit**: Provides a robust interface between human language and robot control systems.
...
-   **Benefit**: Manages complexity, allowing LLMs to focus on high-level reasoning.
...
- [SayCan: Learning to follow language instructions with grounded pretrained models](https://ai.googleblog.com/2022/03/saycan-learning-to-follow-language.html)
- [Inner Monologue: Empowering LLMs with Tools for Sequential Decision-Making](https://arxiv.org/abs/2307.03080)
- [Robotics and Language: Large Language Models for Robotics](https://robotics-and-language.github.io/llms-for-robotics.html)
